...
import { tournamentStatusLabels, winRate } from '../utils/format';
...
export function TeamDetail() {
  ...
  const registeredTournaments = tournaments.filter((item) => item.teams.includes(team.id));
  const waitlistTournaments = tournaments.filter((item) => item.waitlist.includes(team.id));
      ...
      <section className="timeline">
        ...
        {registeredTournaments.map((item) => (
          <div className="timeline-item" key={item.id}>
            {item.name}
            <span className="badge badge--success">{tournamentStatusLabels[item.status]}</span>
          </div>
        ))}
        {waitlistTournaments.map((item) => {
          const position = item.waitlist.indexOf(team.id) + 1;
          return (
            <div className="timeline-item" key={`waitlist-${item.id}`}>
              {item.name}
              <span className="badge badge--warning">候补第 {position} 位</span>
            </div>
          );
        })}
        {registeredTournaments.length === 0 && waitlistTournaments.length === 0 && (
          <div className="timeline-item">暂无赛事参与记录</div>
        )}
      </section>
